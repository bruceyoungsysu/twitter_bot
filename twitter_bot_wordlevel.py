...
# TODO: refine vocab
# TODO: refine the way spliting data


def encode_vocab(text, vocab):
    # return [vocab.index(x)+1 for x in re.split('([.\?.,:!@#\n\s])', text) if x in vocab]
    return [vocab.index(x)+1 for x in word_tokenize(text) if x in vocab]
...
class Char_RNN(object):
    def __init__(self):
        ...
        self.num_steps = 10  # window size
        self.path = './trump_tweets.txt'
        self.skip_step = 500
        self.gen_length = 200
        self.temp = tf.constant(1.5)

    def get_vocab(self, filename='./vocab_tokenized_all.txt'):
        vocab = []
